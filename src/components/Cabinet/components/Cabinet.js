			...
			<div>
				...
				<Paper className={classes.paperTabs}>
					...
					>
						...
						{user.role == 0 ? <Tab label="Создание новостей" /> : <div />}
					</Tabs>
				</Paper>
				{tabIndex === 0 && <UserInfo user={user} classes={classes} />}
				{tabIndex === 1 && (
					<div>
						<Orders classes={classes} orders={orders} />
					</div>
				)}
				{tabIndex === 2 && (
					<div>
						<CreateShedule
							classes={classes}
							createShedule={this.props.createShedule}
							creatingShedule={this.props.creatingShedule}
							creatingSheduleError={this.props.creatingSheduleError}
						/>
					</div>
				)}
				{tabIndex === 3 && (
					<div>
						<CreateNews
							...
							creatingNews={this.props.creatingNews}
							creatingNewsError={this.props.creatingNewsError}
						/>
					</div>
				)}
				<Grid
